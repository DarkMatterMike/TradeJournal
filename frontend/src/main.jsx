    ...
    <div className="app">
      ...
      <div className="page">
        <div className="page__inner">
          ...
          {page === 'intel' && <>
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div><h1 className="page-header__title">Intelligence</h1><p className="page-header__sub">Query your trading history</p></div>
              <BulkIntelligenceButton setStatus={setStatus} onDone={() => { load(); loadStats(); loadPatterns(); }} />
            </div>
            <div className="section">
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
    <div style={{ display: 'flex', gap: 8 }}>
      {!running
        ...
        : <button className="btn btn--danger" onClick={stop}><X size={14} /> Stop</button>
      }
    </div>
    {progress && <div style={{ fontSize: 12, color: 'var(--text-2)', textAlign: 'right' }}>
      <span style={{ color: 'var(--green)', fontWeight: 600 }}>{progress.processed} analyzed</span>
      {progress.remaining !== '?' && <span style={{ color: 'var(--text-3)' }}> · {progress.remaining} remaining</span>}
      {running && <span style={{ color: 'var(--accent)', marginLeft: 8 }}><Loader2 size={11} className="spin" style={{ display: 'inline', verticalAlign: 'middle' }} /> processing…</span>}
      {progress.errors?.length > 0 && <div style={{ color: 'var(--amber)', marginTop: 2 }}>{progress.errors.length} error{progress.errors.length > 1 ? 's' : ''}</div>}
    </div>}
  </div>;
}
